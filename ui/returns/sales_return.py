...
class SalesReturnPage:
    ...
    def _on_bill_select(self, event=None):
        ...
        if not row:
            messagebox.showwarning("Not Found", f"Bill '{bill_no}' not found.")
            return
        self._sale_id = row[0]
    ...
    def _add_return_item_dialog(self, event=None):
        ...
        def _confirm():
            try:
                ...
                messagebox.showerror("Invalid", "Enter a valid integer quantity.", parent=dlg)
                return
    ...
    def _save_return(self):
        if not self._sale_id:
            messagebox.showwarning("No Bill", "Please load a bill first.")
            return
        if not self.return_items:
            messagebox.showwarning("No Items", "Please add items to return.")
            return
        try:
            disc = float(self.discount_entry.get() or 0)
        except ValueError:
        ...
        try:
            ...
            self.conn.rollback()
            messagebox.showerror("Error", f"Failed to save return: {e}")
