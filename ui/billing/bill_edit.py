        ...
        def update():
            try:
                ...
                if new_qty == 0:
                    ...
                    # Preserve existing item discount; re-cap at new base
                    disc = min(float(med.get('medicine_discount', 0)), base)
                    med['medicine_discount'] = disc
                    med['amount'] = round(base - disc, 2)
                self._update_tree()
                self._calculate_total()
                dlg.destroy()
