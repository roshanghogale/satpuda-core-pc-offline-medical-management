...
class PurchasePage(PurchaseNavMixin, PurchaseFormMixin):
    ...
    def load_supplier_details(self, event=None):
        ...
        try:
            ...
            messagebox.showerror("Database Error", "Failed to load supplier details.")
    ...
    def _read_pricing_data(self):
        try:
            ...
            messagebox.showerror("Invalid Input", "Please enter valid rate, MRP and GST.")
            return None
    ...
    def remove_selected_item(self):
        ...
        if 0 <= idx < len(self.purchase_items):
            ...
            self.update_items_tree()
            self.calculate_total()
            messagebox.showinfo("Removed", f"Removed {removed['name']} from list.")
    ...
    def save_purchase(self):
        if not self.supplier_name.get().strip():
            messagebox.showwarning("Missing", "Please enter supplier name.")
            return
        if not self.purchase_items:
            messagebox.showwarning("No Items", "Please add items to the purchase.")
            return
    ...
    def _validate_medicine_fields(self):
        if not self.medicine_name.get().strip():
            messagebox.showwarning("Missing", "Please enter medicine name."); return False
        if not self.medicine_type.get():
            messagebox.showwarning("Missing", "Please select medicine type."); return False
        if not self.batch_no.get().strip():
            messagebox.showwarning("Missing", "Please enter batch number."); return False
        expiry = self.expiry_date.get().strip()
        if not expiry:
            messagebox.showwarning("Missing", "Please enter expiry date (MM/YY)."); return False
        try:
            if '/' not in expiry:
                raise ValueError
            ...
            messagebox.showwarning("Invalid Format", "Expiry must be MM/YY (e.g. 12/26).")
            return False
        return True
