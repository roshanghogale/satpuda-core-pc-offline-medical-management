...
from core.layout_config import PURCHASE_ROWS
...
class PurchaseFormMixin:
    ...
    def _build_medicine_panel(self, parent):
        ...
        self.qty_frame.grid(row=1, column=0, columnspan=4, sticky=tk.EW, padx=5, pady=5)
        self._create_tablet_qty_fields()

        # Fields: (row, col, label, attr, width, next_attr)
        fields = [
            (2, 0, "HSN Code:",    'hsn_code',    20, 'gst_value'),
            (2, 2, "GST %:",       'gst_value',   15, 'mrp'),
            (3, 0, "MRP:",         'mrp',         20, 'rate'),
            (3, 2, "Rate (per strip/unit):", 'rate', 15, 'manufacturer'),
            (4, 0, "Manufacturer:",'manufacturer',30, 'batch_no'),
            (4, 2, "Batch No:",    'batch_no',    15, 'expiry_date'),
            (5, 0, "Expiry (MM/YY):", 'expiry_date', 20, 'schedule'),
            (6, 0, "Content/Drug:", 'content_drug', 30, 'item_discount'),
        ]
        ...
        ttk.Label(mf, text="Schedule:").grid(row=5, column=2, sticky=tk.W, padx=5, pady=5)
        self.schedule = SearchableCombo(mf, values=[s for s in self._schedules if s],
                                        width=15, listbox_height=8)
        self.schedule.grid(row=5, column=3, sticky=tk.EW, padx=5, pady=5)
        self.schedule.next_focus_widget = lambda: self.content_drug.focus()

        ttk.Label(mf, text="Discount %:").grid(row=7, column=0, sticky=tk.W, padx=5, pady=5)
        self.item_discount = ttk.Entry(mf, width=10)
        self.item_discount.grid(row=7, column=1, sticky=tk.EW, padx=5, pady=5)
        ...
        self.add_btn.grid(row=7, column=2, columnspan=2, padx=10, pady=5)
    ...
    def _apply_type_qty_default(self, med_type):
        ...
        try:
            if med_type.lower() in ('tablet', 'bolus'):
                ...
                unit_label = self._sched_unit.get(med_type, '')
    ...
    def _get_auto_unit(self):
        med_type = self.medicine_type.get().lower()
        if med_type == 'vaccine' and hasattr(self, 'vaccine_unit_var'):
            return self.vaccine_unit_var.get()
        return TYPE_UNITS.get(med_type, '')
    ...
    def _build_items_tree(self, parent):
        ...
        cols = ('Medicine','Type','Batch','Expiry','Qty','Pack','Free',
                'Rate','Disc%','GST%','Taxable','GST Amt','Amount')
        widths = {'Medicine':120,'Type':70,'Batch':70,'Expiry':65,'Qty':50,
                  'Pack':50,'Free':45,'Rate':65,'Disc%':45,'GST%':45,
                  'Taxable':70,'GST Amt':65,'Amount':75}
        self.items_tree = ttk.Treeview(items_frame, columns=cols,
    ...
    def _format_unit_display(self, item):
        if item['type'].lower() in ('tablet', 'bolus'):
            ...
            try:
                ...
                return str(tps)
        return str(item.get('quantity_value', ''))
    ...
    def update_items_tree(self):
        ...
        for item in self.purchase_items:
            self.items_tree.insert('', tk.END, values=(
                ...
                item['expiry'],
                f"{item['qty']:.1f}",
                self._format_unit_display(item),
                f"{item['free_qty']:.1f}",
                f"{item['rate']:.2f}",
                f"{item.get('discount_pct', 0):.1f}%",
                f"{item.get('gst_pct', 0):.1f}%",
                f"{item.get('taxable', 0):.2f}",
                f"{item.get('gst_amt', 0):.2f}",
                f"{item.get('item_amount', 0):.2f}",
            ))
